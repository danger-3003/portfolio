...
function LinkButton({ text, onClick, href, target }) {
  return (
    <>
      <a href={href} onClick={onClick} target={target} className="relative inline-flex items-center justify-center px-5 py-0.5 overflow-hidden font-medium text-indigo-600 transition duration-300 ease-out rounded-full shadow-xl group cursor-pointer">
        ...
        <span className="relative text-white pb-0.5 font-nunito">{text}</span>
      </a>
    </>
  )
}
